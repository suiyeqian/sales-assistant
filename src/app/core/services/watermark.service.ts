...
export class WaterMarkService {
  ...
  load(settings = {}): void {
    ...
    if (this.dftSettings.wmk_cols === 0 || (this.dftSettings.wmk_x + sumWidth_x + sumSpace_x > page_width)) {
      let containerWith = page_width - this.dftSettings.wmk_x;
      let pieceWith = this.dftSettings.wmk_width * wmk_cos + this.dftSettings.wmk_x_space;
      this.dftSettings.wmk_cols = Math.round(containerWith / pieceWith);
      let allSpace_x = containerWith - this.dftSettings.wmk_width * wmk_cos * this.dftSettings.wmk_cols;
    ...
    if (this.dftSettings.wmk_rows === 0 || (this.dftSettings.wmk_y + sumWidth_y + sumSpace_y > page_height)) {
      let containerHeight = page_height - this.dftSettings.wmk_y;
      let pieceHeight = this.dftSettings.wmk_height + this.dftSettings.wmk_y_space;
      this.dftSettings.wmk_rows = Math.round(containerHeight / pieceHeight);
